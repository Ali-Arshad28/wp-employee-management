...
class MyEmployees{
    ...
    public function callPluginActivationFunctions(){
        ...
        // Using short code for the content of the page
        $page_content = "[wp-employee-form]";


          
        // Creating a new page with the provided page title
    ...
    public function handleUpdateEmployeeData(){
        ...
        if(!empty($employeeData)){
            ...
            if(!empty($profile_file_image)){
             ...
             /* Can also write this code in place of update() as written below
                $sql = "
                UPDATE {$this->table_name}
                SET name = %s, email = %s, designation = %s, profile_image = %s
                WHERE id = %d
                ";
                $this->wpdb->query($this->wpdb->prepare($sql, $name, $email, $designation, $profile_image_url, $id));
               */
